...
- Login in dev/prod is backend-owned for both email/password and Kakao OAuth.
  ...
  auth sessions.
- `strictPort` 3000 (decision map); pnpm only, never an npm lockfile.

## Run
- test: `pnpm --filter front test`.
- lint: `pnpm --filter front lint` (check) / `pnpm --filter front lint:fix` (autofix). Convention: decision map.
