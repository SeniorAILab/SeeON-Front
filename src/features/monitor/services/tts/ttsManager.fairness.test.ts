...
function systemTest(identity: string): TTSSystemTestAlertInput {
  return {
    identity,
    kind: "SYSTEM_TEST",
    spaceId: null,
...
const text = {
  ...
  systemTest: "System test emergency notification",
};
...
describe("TTSManager first-announcement fairness", () => {
  ...
  it("speaks a never-spoken SYSTEM_TEST before simultaneous higher-severity reannouncements", async () => {
    ...
    // Add SYSTEM_TEST at t=30s, so its first announcement and every high-severity
    // reannouncement become due in exactly the same t=31s tick.
    await vi.advanceTimersByTimeAsync(29_000);
    manager.update([emergency, danger1, danger2, systemTest("event-system-test")], true);
    await vi.advanceTimersByTimeAsync(1_000);
  ...
  it("keeps a brand-new EMERGENCY ahead of cohort-0 CAUTION and all reannouncements", async () => {
    ...
    manager.update(
      [existingEmergency, existingDanger, systemTest("event-system-test-new-emergency"), newEmergency],
      true,
    );
    await vi.advanceTimersByTimeAsync(1_000);
  ...
  it("treats an escalation as a first announcement ahead of reannouncements", async () => {
    ...
    manager.update([existingEmergency, escalated, systemTest("event-system-test-escalation")], true);
    await vi.advanceTimersByTimeAsync(1_000);
